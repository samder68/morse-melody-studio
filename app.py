    ...
    if st.button("🎵 Generate Musical Composition", type="primary", use_container_width=True):
        if user_input.strip():
            with st.spinner("🎼 Composing your musical masterpiece..."):
                # THIS IS THE KEY FIX - updated musical_options structure
                musical_options = {
                    'tempo': tempo,
                    ...
                    'original_message': user_input  # This is crucial for the new algorithm
                }
                        ...
                        with col2:
                            if wav_path and os.path.exists(wav_path):
                                with open(wav_path, "rb") as f:
                                    wav_bytes = f.read()
                                filename = f"morse_composition_{user_input[:10].replace(' ', '_')}_{tempo}bmp.wav"
                                st.download_button(
                                    "🎵 Download Audio File",
                                    wav_bytes,
                                    file_name=filename,
